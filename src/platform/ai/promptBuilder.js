...
const CO_CREATE_APP_INSTRUCTIONS = [
  ...
  'Some Co-Create apps are callback-driven. If the user asks for an adaptive, evolving, interview, questionnaire, coach, teacher, dungeon, story, game master, or progressive experience, the primary user action itself is the AI trigger: answer submitted, choice selected, room completed, next step requested, or challenge solved. Call window.requestAiGeneration immediately when that primary action is validated; do not wait for a separate AI+ button, do not rely only on preload, and do not merely update local UI while saying the IA will be recalled later.',
  'Use semantic triggers for the primary action. Allowed examples include continue_pressed, answer_submitted, choice_selected, question_answered, room_completed, needs_next_step, runtime_generation_requested, ai_request, needs_generation, preload_requested, branch_requested, content_exhausted, and next_requested. Use the most precise trigger instead of defaulting to ai_plus.',
  'Runtime request shape must be clear: window.requestAiGeneration({ trigger: "answer_submitted", state: { /* only meaningful user-facing state */ }, continuationPlan, preload, context: { userIntent: "...", lastUserAction: "...", lastAnswer: "...", userHistory: [...] } }). Request-specific continuationPlan and preload are valid and should be included when the app has them.',
  ...
  'Generated Co-Create apps should log app-level steps at each boundary: answer submitted or primary action validated, requestAiGeneration called, bridge unavailable, raw response received, payload detected, question applied or payload applied, and runtime error. Include the propagated traceId in those app logs when available. The injected Creatia runtime overlay logs host bridge and AI provider steps; do not claim the app contacts OpenAI directly.',
  ...
  'For interview/questionnaire apps, applyRuntimePayload must not merely render runtime cards: if statePatch.currentQuestion exists, replace or append the next visible AI question; if items contains { type: "question" }, use it as the next visible question; clear busy/loading state; keep the answer field usable; append the user answer and AI question to the conversation; never leave “Analyse en attente du rappel IA…” after a successful payload.'
]
...
const RUNTIME_GENERATION_INSTRUCTIONS = [
  ...
  'Do not return raw analysis only. Do not return full HTML unless the runtime explicitly requests full HTML replacement.',
  ...
  'For adaptive interviews and questionnaires, the next generated content must include the next question in runtimePayload.statePatch.currentQuestion or an item with type "question". Use a shape like { "kind": "runtime_generation", "runtimePayload": { "statePatch": { "currentQuestion": "...", "nextAction": "...", "status": "Question générée", "busy": false, "progress": 0.2 }, "items": [{ "type": "question", "title": "...", "text": "..." }] }, "state": {}, "continuationPlan": {}, "preload": [] }.',
...
export function buildRuntimeGenerationPrompt({ runtimeRequest = {}, project = null, designSystem = MANIFESTATION_DESIGN_SYSTEM }) {
  const payload = {
    ...
    instruction: 'Generate only runtimePayload for this trigger. Include traceId, trigger, currentState, concise continuationPlan, preload, and context in the runtime prompt target. Do not return raw analysis only. Do not return or rewrite the full HTML app unless explicitly requested by the runtime. For interviews/questionnaires, include the next question in runtimePayload.statePatch.currentQuestion or runtimePayload.items[{ type: "question" }].'
  }
