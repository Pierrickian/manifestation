...
const CO_CREATE_APP_INSTRUCTIONS = [
  ...
  'continuationPlan must only describe the runtime AI role in one or two short sentences, for example { "runtimeRole": "Generate new inspiration cards when AI+ is pressed." }.',
  ...
  'Do not generate future content, future prompts, prepared runtime generations, future screens, rooms, cards, stories, or citations.',
  'Do not precompute future runtime generations. Runtime AI will be recalled later with fresh requested context.',
...
export function buildRuntimeGenerationPrompt({ request = {}, project = null, designSystem = MANIFESTATION_DESIGN_SYSTEM } = {}) {
  const payload = {
    ...
    originalRequest: project?.creationRequest || request.context?.originalRequest || '',
    applicationState: request.state || project?.applicationState || {},
    userHistory: request.context?.userHistory || request.context?.actions || [],
    project: project ? {
