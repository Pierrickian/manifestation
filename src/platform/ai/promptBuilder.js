...
const STRUCTURED_APP_INSTRUCTIONS = [
  ...
  'Co-Create generated apps must expose at least one runtime result consumer. Prefer window.applyRuntimePayload = function(runtimePayload) { ... } for new applications. Alternatively expose window.onAiResponse = function(event) { ... }. Legacy consumers window.applyGeneratedContent(payload) and window.applyGeneratedRoom(roomOrPayload) may also be used, but new code should consume event.data.runtimePayload.',
